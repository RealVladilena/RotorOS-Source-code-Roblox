-- ============================================================
-- UI BUILDER — ENERCON SCADA REMOTE 4 (Avec Diagnostic de Courbe)
-- ============================================================
...
local P = {
	...
	TreeSelTxt  = Color3.fromHex("#FFFFFF"),
...
local function AddCorner(parent, radius) return Make("UICorner", {CornerRadius = UDim.new(0, radius)}, parent) end
...
local function MakeLabel(parent, text, pos, size, align, color, font)
	return Make("TextLabel", {
		...
		TextXAlignment = align or Enum.TextXAlignment.Left, BackgroundTransparency = 1, TextScaled = false
	})
end
...
local function getTurbineInfo(turbineRoot)
	...
	return string.gsub(topModel.Name, " %(start new%)", ""), parkFolder
end

-- ============================================================
-- 1. FENÊTRE PRINCIPALE
-- ============================================================
local gui = Make("ScreenGui", {Name = "E82FrScada", ResetOnSpawn = false, Enabled = false}, playerGui)
...
MakeLabel(topBar, "  ⚡ E82Fr SCADA System", UDim2.new(0, 15, 0, 0), UDim2.new(1, -50, 1, 0), Enum.TextXAlignment.Left, P.TopText, FontBold)
local closeBtn = Make("TextButton", {Parent = topBar, Position = UDim2.new(1, -40, 0, 8), Size = UDim2.new(0, 24, 0, 24), Text = "X", BackgroundColor3 = P.StatusRed, TextColor3 = Color3.new(1,1,1), Font = FontBold, TextSize = 14})
...
local treeScroll = Make("ScrollingFrame", {Parent = leftPanel, Size = UDim2.new(1, 0, 1, -10), Position = UDim2.new(0,0,0,10), BackgroundTransparency = 1, ScrollBarThickness = 4, CanvasSize=UDim2.new(0,0,0,0), AutomaticCanvasSize=Enum.AutomaticSize.Y})
local treeLayout = Make("UIListLayout", {Parent = treeScroll, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 2)})
...
local tableScroll = Make("ScrollingFrame", {Parent = rightPanel, Position = UDim2.new(0, 0, 0, 40), Size = UDim2.new(1, 0, 1, -95), BackgroundTransparency = 1, BorderSizePixel = 0, ScrollBarThickness = 6, CanvasSize=UDim2.new(0,0,0,0), AutomaticCanvasSize=Enum.AutomaticSize.Y})
...
Make("Frame", {Parent = bottomBar, Size = UDim2.new(0, 10, 1, 0), Position = UDim2.new(0, 0, 0, 0), BackgroundColor3 = P.BgLeft, BorderSizePixel = 0})
...
local dCloseBtn = Make("TextButton", {Parent = dTopBar, Position = UDim2.new(1, -40, 0, 13), Size = UDim2.new(0, 24, 0, 24), Text = "X", BackgroundColor3 = P.BgMain, TextColor3 = P.CellTxt, Font = FontBold, TextSize = 12, BorderSizePixel = 0})
...
local mast = Make("Frame", {Parent = tModel, Position = UDim2.new(0.5, -2, 0.2, 0), Size = UDim2.new(0, 4, 0.8, 0), BackgroundColor3 = Color3.fromHex("#D1D8E0"), BorderSizePixel = 0})
local nacelle = Make("Frame", {Parent = tModel, Position = UDim2.new(0.5, -12, 0.2, -6), Size = UDim2.new(0, 24, 0, 12), BackgroundColor3 = Color3.fromHex("#A5B1C2"), BorderSizePixel = 0})
local hub = Make("Frame", {Parent = tModel, Position = UDim2.new(0.5, 0, 0.2, 0), Size = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1})
local function CreateBlade(rot)
	local bladeBase = Make("Frame", {Parent = hub, Position = UDim2.new(0, 0, 0, 0), Size = UDim2.new(0, 0, 0, 0), Rotation = rot, BackgroundTransparency = 1})
	local blade = Make("Frame", {Parent = bladeBase, Position = UDim2.new(0, -2, 0, -55), Size = UDim2.new(0, 4, 0, 55), BackgroundColor3 = Color3.fromHex("#7A8288"), BorderSizePixel = 0})
end
CreateBlade(0) CreateBlade(120) CreateBlade(-120)
...
local pInputBox  = Make("TextBox", {Parent = dContent, Position = UDim2.new(0, 150, 0, 270), Size = UDim2.new(0, 60, 0, 32), Text = "", PlaceholderText = "°", BackgroundColor3 = P.RowOdd, Font = FontBold, TextSize = 13, TextColor3 = P.CellTxt, BorderSizePixel = 0})
...
local function buildUI()
	...
	for parkName, machs in pairs(parks) do
		...
		for _, m in ipairs(machs) do
			local i = m.index
			local machineId = 3747 + i

			-- 🟢 ICI ON RÉCUPÈRE LE TAG "CURVE" POUR L'AFFICHAGE DU NOM
			local realName = m.name
			local hasCurve = m.obj:GetAttribute("HasPowerCurve")
			if hasCurve then
				realName = realName .. " [CURVE]"
			end
			...
			local tLbl = MakeLabel(tRow, "⚙ " .. realName, UDim2.new(0, 15, 0, 0), UDim2.new(1, -15, 1, 0), Enum.TextXAlignment.Left, P.TopText)
			...
			MakeLabel(row, tostring(machineId), UDim2.new(0, 15, 0, 0), UDim2.new(COLS[1].w, -15, 1, 0), Enum.TextXAlignment.Left, Color3.fromHex("#7F8C8D"))
			...
			local tMaxKw = m.obj:GetAttribute("MaxPowerVal")
			if not tMaxKw or tMaxKw == 0 then tMaxKw = 4500 end

			local pwrCont = Make("Frame", {Parent = row, Position = UDim2.new(COLS[1].w+COLS[2].w+COLS[3].w, 15, 0, 0), Size = UDim2.new(COLS[4].w, -25, 1, 0), BackgroundTransparency=1})
			local fPwr, tPwr, maxP = MakeBar(pwrCont, P.BarPower, tMaxKw)

			local wndCont = Make("Frame", {Parent = row, Position = UDim2.new(COLS[1].w+COLS[2].w+COLS[3].w+COLS[4].w, 15, 0, 0), Size = UDim2.new(COLS[5].w, -25, 1, 0), BackgroundTransparency=1})
			local fWnd, tWnd, maxW = MakeBar(wndCont, P.BarWind, 25)

			local rpmCont = Make("Frame", {Parent = row, Position = UDim2.new(COLS[1].w+COLS[2].w+COLS[3].w+COLS[4].w+COLS[5].w, 15, 0, 0), Size = UDim2.new(COLS[6].w, -25, 1, 0), BackgroundTransparency=1})
			local fRpm, tRpm, maxR = MakeBar(rpmCont, P.BarRpm, 20)
			...
			table.insert(rowFrames, {
				row=row, statusLbl=statusLbl, turbine=m.obj, realName=realName, parkName = parkName,
				fPwr=fPwr, tPwr=tPwr, mPwr=maxP, fWnd=fWnd, tWnd=tWnd, mWnd=maxW, fRpm=fRpm, tRpm=tRpm, mRpm=maxR
			})
		end
...
-- ============================================================
-- 4. ÉVÈNEMENTS BOUTONS
-- ============================================================
local currentTarget = 1
local targetModes = {"ALL", "Blade 1", "Blade 2", "Blade 3"}

pTargetBtn.MouseButton1Click:Connect(function() currentTarget = currentTarget % 4 + 1 pTargetBtn.Text = "Target: " .. targetModes[currentTarget] end)
pForceBtn.MouseButton1Click:Connect(function() local p = tonumber(pInputBox.Text) local t = turbinesList[activeTargetIndex] if p and t then event:FireServer("SetManualPitch", t, p, currentTarget - 1) pInputBox.Text = "" end end)
pResetBtn.MouseButton1Click:Connect(function() local t = turbinesList[activeTargetIndex] if t then event:FireServer("ResetPitch", t) end end)
togglePwrBtn.MouseButton1Click:Connect(function() local t = turbinesList[activeTargetIndex] if t then local val = t:FindFirstChild("IsOn", true) if val then event:FireServer("TogglePower", t, not val.Value) end end end)
...
ProximityPromptService.PromptTriggered:Connect(function(prompt, who)
	if prompt.Name == "TurbinePrompt" and who == player then
		pcall(function()
			local temp = {}
			for _, obj in ipairs(workspace:GetDescendants()) do
				if obj:GetAttribute("IsWindTurbine") == true then
					local dup = false for _, t in ipairs(temp) do if t == obj then dup = true break end end
					if not dup then table.insert(temp, obj) end
				end
			end
			turbinesList = temp
			table.sort(turbinesList, function(a, b) return a.Name < b.Name end)
			buildUI()
			gui.Enabled = true
		end)
	end
...
-- ============================================================
-- 5. BOUCLE DE MISE À JOUR (Propre)
-- ============================================================
local rotorAngle = 0
...
RunService.RenderStepped:Connect(function()
	...
	for i, rd in ipairs(rowFrames) do
		local t = rd.turbine
		local stateVal = t:FindFirstChild("CurrentState", true)
		local state = stateVal and stateVal.Value or "STOPPED"
		local power = t:GetAttribute("CurrentPower") or 0
		local rpm = t:GetAttribute("CurrentRPM") or 0
		local wind = (t:GetAttribute("RealWindSpeed") or 0) / 3.6 

		-- Sécurité basique
		if type(power) ~= "number" or power ~= power then power = 0 end
		if type(rpm) ~= "number" or rpm ~= rpm then rpm = 0 end
		if type(wind) ~= "number" or wind ~= wind then wind = 0 end

		globalPower = globalPower + power
		parkPowerMap[rd.parkName] = (parkPowerMap[rd.parkName] or 0) + power

		if state == "RUNNING" then rd.statusLbl.Text = "● RUN" rd.statusLbl.TextColor3 = P.StatusGreen
		elseif state == "STARTING" then rd.statusLbl.Text = "● START" rd.statusLbl.TextColor3 = P.StatusYel
		elseif state == "LACK_OF_WIND" then rd.statusLbl.Text = "● WAIT" rd.statusLbl.TextColor3 = P.StatusGray
		else rd.statusLbl.Text = "● STOP" rd.statusLbl.TextColor3 = P.StatusRed end

		-- Calcul de remplissage des barres avec sécurité absolue contre les valeurs nulles
		local maxP = rd.mPwr
		if not maxP or maxP <= 0 then maxP = 4500 end

		local pRatio = 0
		if maxP > 0 then pRatio = math.clamp(power / maxP, 0, 1) end

		local wRatio = 0
		if rd.mWnd > 0 then wRatio = math.clamp(wind / rd.mWnd, 0, 1) end

		local rRatio = 0
		if rd.mRpm > 0 then rRatio = math.clamp(rpm / rd.mRpm, 0, 1) end

		rd.fPwr.Size = UDim2.new(pRatio, 0, 1, 0) 
		rd.tPwr.Text = string.format("%.0f", power)

		rd.fWnd.Size = UDim2.new(wRatio, 0, 1, 0) 
		rd.tWnd.Text = string.format("%.1f", wind)

		rd.fRpm.Size = UDim2.new(rRatio, 0, 1, 0) 
		rd.tRpm.Text = string.format("%.1f", rpm)
	end

	for pName, lbl in pairs(parkLabels) do
		local pVal = parkPowerMap[pName] or 0
		if pVal > 1000 then lbl.Text = string.format("📂 %s [ %.2f MW ]", pName, pVal / 1000)
		else lbl.Text = string.format("📂 %s [ %.0f kW ]", pName, pVal) end
	end

	if globalPower > 1000 then globalPwrLabel.Text = string.format("⚡ TOTAL: %.2f MW", globalPower / 1000)
	else globalPwrLabel.Text = string.format("⚡ TOTAL: %.0f kW", globalPower) end
	...
	if detailBg.Visible and activeTargetIndex > 0 then
		...
		local stateVal = t:FindFirstChild("CurrentState", true)
		local state = stateVal and stateVal.Value or "STOPPED"
		if state == "RUNNING" then dStatusLbl.Text = "● Operational" dStatusLbl.TextColor3 = P.StatusGreen
		elseif state == "STARTING" then dStatusLbl.Text = "● Starting up" dStatusLbl.TextColor3 = P.StatusYel
		elseif state == "LACK_OF_WIND" then dStatusLbl.Text = "● Standby (Wind)" dStatusLbl.TextColor3 = P.StatusGray
		...
		local b1 = t:FindFirstChild("BladeAngle1", true)
		local b2 = t:FindFirstChild("BladeAngle2", true)
		local b3 = t:FindFirstChild("BladeAngle3", true)
		dPitch1.Text = b1 and string.format("%.1f", b1.Value) or "--"
		dPitch2.Text = b2 and string.format("%.1f", b2.Value) or "--"
		dPitch3.Text = b3 and string.format("%.1f", b3.Value) or "--"

		local isOn = t:FindFirstChild("IsOn", true)
		if isOn and isOn.Value then 
			togglePwrBtn.Text = "SHUTDOWN TURBINE" 
			togglePwrBtn.BackgroundColor3 = P.BtnStop 
			togglePwrBtn.TextColor3 = P.StatusRed
		else 
			togglePwrBtn.Text = "START TURBINE" 
			togglePwrBtn.BackgroundColor3 = P.BtnStart 
			togglePwrBtn.TextColor3 = P.StatusGreen
		end
	end
end)
